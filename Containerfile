...
RUN --mount=type=cache,dst=/var/cache \
    ...
    dnf5 -y config-manager setopt "terra-mesa".exclude="mesa-vulkan-drivers*" && \
    dnf5 -y config-manager setopt "*bazzite*".priority=2 && \
    eval "$(/ctx/dnf5-setopt setopt '*negativo17*' priority=4 exclude='mesa-* *xone*')" && \
    dnf5 -y config-manager setopt "*fedora*".exclude="kernel-core-* kernel-modules-* kernel-uki-virt-* steam noopenh264" && \
